...
class Printer:
    ...
    def err(self, msg):
        print self.pre() + self.RED + msg + self.ENDC
        return self.pre() + msg + "\n"
...
'''
Build the tar file, that is pypi compatible
'''
...
'''
Installs the passed 'package' in the given virtualenv
'''
...
'''
Creates a debian package. Refer scripts/basher for details.
'''
...
'''
Creates the tar file. Refer scripts/basher for details.
'''
...
def main():
    ...
    if args.clean:
        p.log("Cleaning working directory." +  p.RED + " Beware 'clean' build takes lot of time!" + p.ENDC)
    ...
    for repo in config.get("git-repos"):
        ...
        if os.path.isdir(repo_path):
            ...
            git.pull(uri, repo_path, logfile)
        else:
            p.debug("  * Creating directory %s for cloning" % (repo))
            os.makedirs(repo_dir)
            p.log("  * Cloning '%s' under '%s'" % (uri, repo_dir))
            git.clone(uri, repo_dir, logfile)
        path = repo.get('path')
        git.checkout(str(path), repo_path, logfile)
